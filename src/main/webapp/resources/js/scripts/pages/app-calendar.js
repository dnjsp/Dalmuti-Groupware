...
//	eventLabel.val(info.event._def.extendedProps.schdulSeCode).prop("selected", true);
//	console.log(info.event._def.extendedProps.schdulSeCode);	//S1, S2, S3
	eventLabel.val(info.event._def.extendedProps.schdulSeCode).trigger('change');
...
//			dataType:'json',
  ...
  function fetchEvents(info, successCallback) {
	...
	var lists = [];
	$("input[name='view_check']:checked").each(function(i){   //jQuery로 for문 돌면서 check 된값 배열에 담는다
		lists.push($(this).val());
	});
	var selectAll = $("#select-all").val();
